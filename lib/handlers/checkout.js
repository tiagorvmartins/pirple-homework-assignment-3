 ...
 */
 ...
 // Optional data: none
 checkoutHandler._checkout.get = function(data, callback){
   // Get tokenId and userIdentifier from header
   var token = typeof(data.queryStringObject.tokenId) == 'string' ? data.queryStringObject.tokenId : false;
   var userIdentifier = typeof(data.queryStringObject.userId) == 'string' ? data.queryStringObject.userId : false;
   ...
   if(token && userIdentifier){
     tokenHelper.verifyToken(token, userIdentifier, function(tokenIsValid){
       if(tokenIsValid){
         ...
         var stripeToken = typeof(data.queryStringObject.stripeToken) == 'string' && data.queryStringObject.stripeToken.trim().length > 0 ? data.queryStringObject.stripeToken : false;
